...
create or replace function auth.tenant_id() returns uuid language sql stable security definer set search_path=public,auth as $$
...
create or replace function public.tiene_permiso(permiso text) returns boolean language sql stable security definer set search_path=public as $$
 ...
 where u.auth_user_id=auth.uid() and u.tenant_id=auth.tenant_id() and (p.codigo=permiso or p.codigo='*'))
$$;
...
create table if not exists public.gestion_registros (
 id uuid primary key default gen_random_uuid(), tenant_id uuid not null default auth.tenant_id() references public.tenants(id) on delete cascade,
...
begin new.tenant_id:=coalesce(auth.tenant_id(),new.tenant_id); if new.creado_por is null then new.creado_por:=auth.uid(); end if; return new; end $$;
...
do $$ declare t text; begin
 foreach t in array array['tenants','tenant_branding','gestion_sucursales','gestion_usuarios','gestion_roles','gestion_roles_permisos','gestion_usuarios_roles','gestion_registros','gestion_clientes','gestion_mascotas','gestion_turnos','gestion_consultas','gestion_items','gestion_caja_sesiones','gestion_auditoria'] loop
  ...
  execute format('create policy %I on public.%I for select to authenticated using (%s)',t||'_select',t,case when t='tenants' then 'id=auth.tenant_id()' else 'tenant_id=auth.tenant_id()' end);
  if t<>'gestion_auditoria' then
   execute format('drop policy if exists %I on public.%I',t||'_insert',t);
   execute format('create policy %I on public.%I for insert to authenticated with check (%s)',t||'_insert',t,case when t='tenants' then 'id=auth.tenant_id()' else 'tenant_id=auth.tenant_id()' end);
   execute format('drop policy if exists %I on public.%I',t||'_update',t);
   execute format('create policy %I on public.%I for update to authenticated using (%s) with check (%s)',t||'_update',t,case when t='tenants' then 'id=auth.tenant_id()' else 'tenant_id=auth.tenant_id()' end,case when t='tenants' then 'id=auth.tenant_id()' else 'tenant_id=auth.tenant_id()' end);
  end if;
 end loop;
end $$;
...
create policy storage_tenant_select on storage.objects for select to authenticated using(bucket_id in ('logos','mascotas','estudios','documentos','firmas') and (storage.foldername(name))[1]=auth.tenant_id()::text);
drop policy if exists storage_tenant_insert on storage.objects;
create policy storage_tenant_insert on storage.objects for insert to authenticated with check(bucket_id in ('logos','mascotas','estudios','documentos','firmas') and (storage.foldername(name))[1]=auth.tenant_id()::text);
drop policy if exists storage_tenant_update on storage.objects;
create policy storage_tenant_update on storage.objects for update to authenticated using((storage.foldername(name))[1]=auth.tenant_id()::text) with check((storage.foldername(name))[1]=auth.tenant_id()::text);
drop policy if exists storage_tenant_delete on storage.objects;
create policy storage_tenant_delete on storage.objects for delete to authenticated using((storage.foldername(name))[1]=auth.tenant_id()::text);
