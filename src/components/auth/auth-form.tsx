...
export function AuthForm({mode}:{mode:"login"|"registro"}){
  ...
  async function sendMagicLink(event:React.MouseEvent<HTMLButtonElement>){
    const form=event.currentTarget.form;if(!form||loading)return;
    const parsed=loginSchema.pick({email:true}).safeParse({email:new FormData(form).get("email")});
    if(!parsed.success){setError(parsed.error.issues[0]?.message);return}
    setLoading(true);setError(undefined);
    const {error:requestError}=await createClient().auth.signInWithOtp({email:parsed.data.email,options:{emailRedirectTo:`${window.location.origin}/auth/callback`}});
    if(requestError)setError(requestError.message);else setSuccess("Te enviamos un enlace de acceso. Revisá tu email.");
    setLoading(false);
  }
